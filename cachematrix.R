## Code for Programming Assignemnt 2 of the R Programming course.
...
## The first function creates an object  that will store both a matrix and its inverse together 
## Calculation of the inverse is postponed until it is first required.
...
##   3. The function getInverse that returns the inverse of the matrix.
...
cacheSolve <- function(x, ...) {
  ## Get the inverse an it is is not NULL return it.
  inv <-x$getInverse()
  if(!is.null(inv)) return(inv)
   
  ## Inverse has not  yet be calculated so we do so then cache it.
  m <-x$get()
  inv <- solve(m, ...)
  print(inv)
  x$setInverse(inv)
